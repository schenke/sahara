    ...
    void Setup(std::string EventClass,int EventID){
        ...
        for(int xIndex=0;xIndex<NsX;xIndex++){
            for(int yIndex=0;yIndex<NsY;yIndex++){
                ...
                std::string fname=EventClass + "/EventID" + std::to_string(EventID) + "/SMEAREDDISTRIBUTION/SmearedDistributionT0.2005fm" + std::to_string(xLow) + "X" + std::to_string(xHigh) + "_" + std::to_string(yLow) + "Y" + std::to_string(yHigh) + "ID" + std::to_string(EventID) + ".txt";
